...
const CAP_LABELS = {
  ...
  personal: "Personal Trader (mentoria de 30 dias)",
};
const PAGE_CAPS = ["panorama", "carteira", "carteira_write", "conselheiro", "trades", "portfolio", "estudos", "personal"];
...
function can(session, cap) {
  ...
  // Moderador tem todas as páginas implicitamente (espelha effectivePerms no
  // backend). Sem isto, uma sessão em cache de antes de uma página nova ficaria
  // sem ela até o próximo login.
  if (session.role === "moderator" && PAGE_CAPS.includes(cap)) return true;
  ...
  return (
    <>
      ...
      <Shell session={session} active={current} onNavigate={setActive} onLogout={logout} onUpdateSession={setSession}>
        ...
        {current === "personal" && <PersonalScreen session={session} />}
